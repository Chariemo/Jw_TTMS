...
function getStudio()
{
	document.getElementById("fom").action="/Jw_TTMS/StudioServlet?method=search&from=addseat";
		...
		for (var r = 1; r <= row_count; r++) {
			for (var c = 1; c <= col_count; c++) {
				...
				var checkBox="<input type='checkbox' " + 
				  " name= 'seat'" + " checked='checked' onclick ='return false' value=".concat(value).concat(">");
...
<body class="ContentBody">
  <form action="/Jw_TTMS/SeatServlet?method=add&from=addseat" method="post" name="fom" id="fom" onsubmit = "">
<div class="MainDiv">
...
<div class="container">
    <div class="demo clearfix">
        ...
        <p>请选择演出厅：</p>
            ...
            <option value = "" selected = "selected">点击查找获取演出厅</option>
            			...
            			if (liststu != null && liststu.size() > 0) {
            				for (int m = 0; m < liststu.size(); m++) {
            					if (liststu.get(m).getStudio_flag() == 0) {
            						%>
            ...
            <p> ${result}</p>
            <div id="legend" class="seatCharts-legend"><ul class="seatCharts-legendList"><li class="seatCharts-legendItem"><div><span><input type = "checkbox" checked = "checked" disabled = "disabled"></span></div><span class="seatCharts-legendDescription">已选择创建</span></li><li class="seatCharts-legendItem"><div><span><input type = "checkbox" disabled = "disabled"></span></div><span class="seatCharts-legendDescription">未选择创建</span></li></ul></div>
        </div>
    </div>
</div>			
